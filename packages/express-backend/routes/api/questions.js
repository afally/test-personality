...
router.post("/addquestion", (req, res) => {
  ...
  if(questionz){
    ...
    Question.findOne({ number: req.body.number }).then(question => {
        if (question) {
...
router.put('/updatequestion/:questionId', (req, res) => {
  ...
  Question.findOneAndUpdate(questionId, { question, optiona, optionb,optionc,optiond }, { new: true })
...
router
    .delete('/:id', (req, res) => {
        Question.findOneAndDelete(req.params.id)//we would get id from the URL
            .then(deletedquestion => { console.log(deletedquestion)})
